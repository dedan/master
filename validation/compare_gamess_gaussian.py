...
print len(set(gamess_problems.keys()).difference(set(gaussian_problems.keys())))
print set(gamess_problems.keys()).difference(set(gaussian_problems.keys()))
...
for_both = list(set(gamess.keys()).intersection(set(gaussian.keys())))
for i in range(10):
    fig = plt.figure()
    ax = fig.add_subplot(111)

    ax.bar(gaussian[for_both[i]]['freq'], gaussian[for_both[i]]['ir'], edgecolor='b', width=3)
    plt.hold(True)
    ax.bar(gamess[for_both[i]]['freq'], -np.array(gamess[for_both[i]]['ir']), edgecolor='r', width=3)
    ax.set_yticklabels([])
